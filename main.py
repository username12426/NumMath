...
def newmark_simmulation(parameters, indexes, static_solution):
    beta = parameters.beta
    gamma = parameters.gamma
    eta = parameters.eta

    M_e = getMe(parameters, indexes)
    M = getM(parameters, indexes)
    S = getS(parameters, indexes)
    S_e = getSe(parameters, indexes)
    v_e = getve(parameters, indexes)  # reinitialize the v_e vector, because the load has changed
    v_q = getvq(parameters, indexes)
    v_n = getvn(parameters)
    C = getC(parameters)
...
if __name__ == "__main__":
    ...
    Task 15
    '''
    my = lambda x_: x_ ** 0  # Längenspezifische Masse in kg/m
    E = lambda x_: x_ ** 0 # Elastizitätsmodul in N/m^2
    I = lambda x_: x_ ** 0  # Flächenträgheitsmoment in m^4
    q = lambda x_: x_ ** 0  # Streckenlast in N/m
    ...
    class Parameters:
        ...
        def set_n(self, new_n):
            ...
            self.B = np.array([[0, 1, 0],
                               ...
                               [self.n, 4, 0]])
            # Note:
            # Be careful with setting deviations at specific n, may cause error in convergence analysis!
    ...
    params = Parameters()
    params.set_n(params.n)  # Set Initial B for task 10

    '''
    Task 10
    '''

    print(f'Start generating Plot of task(10, 11)')
    ...
    # Build the mat for the DGL n = 100
    n_large = 100
    ...
    # For small scaling
    # ax[0].set_ylim(-max(alpha_e_static_solution_n3_arr), min(alpha_e_static_solution_n3_arr))
    ...
    # Note:
    # You have to be careful with high period periodic line loads and small n
    # This may lead to something similar to an aliasing effect
    ...
    print("Start generating animation task(12)")
    print("Set q to zero")

    params.q = lambda x_: 0  # Set Loads to zero for free swinging beam
    idx_12 = Indices(getindizes(params))

    # calculate solution with newmark procedure
    a_p_animation, total_energy_newmark = newmark_simmulation(params, idx_12, alpha_e_static_solution_n3_arr)
    # generate the plot
    ...
    print("Start error analysis task(13)")
    print("Set {q, my, E and I} to one")

    # set parameters
    params.q = lambda x_: 1  # set Lamda to a constant function, because this task only works with constant q
    params.my = lambda x_: 1
    params.E = lambda x_: 1
    params.I = lambda x_: 1
    ...
    fig.text(0.5, 0, "*These Plots are run for a constant q, my, E and I! \n"
                         "Analytical solution only works for constant values \n"
                       "Analytical solution only for simple canter leaver ", ha='center', fontsize=10)
    ...
    print("Reset {q, my, E and I} to predefined functions")
    ...
    print("Start energy calculation task (14)")
    print("Set q to zero")

    params.q = lambda x_: 0    # We want to look at a free swinging beam!
    idx_14 = Indices(getindizes(params))
    # calculate values for different cases
    t = np.linspace(0, 100, params.n_p)
    # a)
    total_energy_a = newmark_simmulation(params, idx_14, alpha_e_static_solution_n3_arr)
    # b)
    params.eta = 1
    total_energy_b = newmark_simmulation(params, idx_14, alpha_e_static_solution_n3_arr)
    # c)
    params.beta = 1
    params.gamma = 1
    params.eta = 0.1
    total_energy_c = newmark_simmulation(params, idx_14, alpha_e_static_solution_n3_arr)
    # d)
    params.eta = 1
    total_energy_d = newmark_simmulation(params, idx_14, alpha_e_static_solution_n3_arr)
    ...
    print("Start generating high-res-animation task(20)")
